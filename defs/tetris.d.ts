...
export interface TetrisTactile extends TetrisControl {
    ...
    /**
     * Cooldown defines waiting periods, in milliseconds, since a user first gave input on a button before they’re
     * permitted to do so again. This is used most often in tandem with the Cost to provide a voting mechanism.
     */
    cooldown: {
        press: number;
    };
...
export interface TetrisJoystick extends TetrisControl {
    ...
    analysis: {
        /**
         * Coords is currently the only archetype of analysis that can be done, which will report optionally on the
         * average and deviation of the X and Y coordinates on the joystick.
         */
        coords: {
            mean: boolean;
            stdDev: boolean;
